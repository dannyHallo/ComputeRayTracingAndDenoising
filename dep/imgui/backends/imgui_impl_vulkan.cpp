...
//  2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not being honored + removed a bunch of duplicate
//  code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2022-10-04: Vulkan: Added experimental
//  ImGui_ImplVulkan_RemoveTexture() for api symetry. (#914, #5738). 2022-01-20: Vulkan: Added support for ImTextureID as
//  VkDescriptorSet. User need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define ImTextureID
//  ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a full-viewport to reduce likehood of issues
//  with people using VK_DYNAMIC_STATE_SCISSOR in their app without calling vkCmdSetScissor() explicitly every frame. 2021-06-29:
...
//  to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes are not multiple of
//  VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change blending equation to preserve alpha in output buffer.
...
//  VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline parameter to ImGui_ImplVulkan_RenderDrawData (default to one passed
//  to ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial frame has no vertices. 2020-04-26: Vulkan: Fixed edge
//  case where render callbacks wouldn't be called if the ImDrawData didn't have vertices. 2019-08-01: Vulkan: Added support for
//  specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to use,
//  default is non-multisampled as before. 2019-05-29: Vulkan: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added support for special ImDrawCallback_ResetRenderState
//  callback to reset render state. 2019-04-04: *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields in
//  ImGui_ImplVulkan_InitInfo, required for initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added
//  ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added VkInstance argument to ImGui_ImplVulkanH_CreateWindow()
//  optional helper. 2019-04-04: Vulkan: Avoid passing negative coordinates to vkCmdSetScissor, which debug validation layers do
//  not like. 2019-04-01: Vulkan: Support for 32-bit index buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan: Viewport
//  and clipping rectangles correctly using draw_data->FramebufferScale to allow retina display. 2018-11-30: Misc: Setting up
//  io.BackendRendererName so it can be displayed in the About Window. 2018-08-25: Vulkan: Fixed mishandled
//  VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the parameters to ImGui_ImplVulkan_RenderDrawData() to be
//  consistent with other backends. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan
//  example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix and clipping
//  rectangle. 2018-03-03: Vulkan: Various refactor, created a couple of ImGui_ImplVulkanH_XXX helper that the example can use and
//  that viewport support will use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields
//  to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback,
//  ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06: Misc: Removed call to ImGui::Shutdown()
//  which is not available from 1.60 WIP, user needs to call CreateContext/DestroyContext themselves. 2017-05-15: Vulkan: Fix
//  scissor offset being negative. Fix new Vulkan validation warnings. Set required depth member for buffer image copy.
//  2016-11-13: Vulkan: Fix validation layer warnings and errors and redeclare gl_PerVertex.
//  2016-10-18: Vulkan: Add location decorators & change to use structs as in/out in glsl, update embedded spv (produced with
...
//   IMGUI_VULKAN_FUNC_MAP_MACRO(vkUnmapMemory) \ IMGUI_VULKAN_FUNC_MAP_MACRO(vkUpdateDescriptorSets)
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, bd->RenderPass, v->MSAASamples, &bd->Pipeline,
                                  bd->Subpass);
