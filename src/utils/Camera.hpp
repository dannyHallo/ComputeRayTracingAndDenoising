...
public:
  Camera(Window *window, glm::vec3 camPosition = glm::vec3(0.F, 0.F, 0.F),
         ...
         float vFov = 60.F)
      : mPosition(camPosition), mWorldUp(wrdUp), mYaw(camYaw), mPitch(camPitch), mVFov(vFov),
        mWindow(window) {
    updateCameraVectors();
  }
  ...
  [[nodiscard]] glm::dmat4 getViewMatrixDouble() const {
    glm::dvec3 dPosition = glm::dvec3(mPosition);
    glm::dvec3 dFront    = glm::dvec3(mFront);
    glm::dvec3 dUp       = glm::dvec3(mUp);
    return glm::lookAt(dPosition, dPosition + dFront, dUp);
  }
