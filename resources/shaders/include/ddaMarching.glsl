...
bool ddaMarching(out ivec3 oHitChunkPosOffset, out uvec3 oHitChunkLookupOffset,
                 inout float ioOptimizedDistance, vec3 o, vec3 d) {
  o += d * ioOptimizedDistance; // to skip certain number of chunks
  ...
  for (int i = 0; i <= MAX_DDA_ITERATION; i++) {
    ...
    if (inChunkRange(lookupPos)) {
      ...
      if (hasChunk(lookupPos)) {
        ...
        ioOptimizedDistance += min(sideDist.x, min(sideDist.y, sideDist.z)) + 1e-4;
        return true;
      }
    }
